...
public:
  ...
  MOCK_METHOD(WiFiMode_t, getMode, (), (override));
  MOCK_METHOD(bool, addAP, (const char *ssid, const char *passphrase),
              (override));
...
TEST(WiFiConnector, test_connect_resolves_if_WiFi_is_already_connected) {
  ...
  EXPECT_CALL(mock, getMode()).Times(1).WillRepeatedly(Return(WIFI_STA));
...
TEST(WiFiConnector,
  ...
  EXPECT_CALL(mock, getMode()).WillRepeatedly(Return(WIFI_AP_STA));
...
TEST(
  ...
  EXPECT_CALL(mock, getMode()).Times(1).WillRepeatedly(Return(WIFI_STA));
  EXPECT_CALL(mock, status()).Times(1).WillRepeatedly(Return(WL_CONNECTED));
  EXPECT_CALL(mock, addAP(Eq(ssid), Eq(password)))
      .Times(1)
...
TEST(
  ...
  EXPECT_CALL(mock, getMode()).WillRepeatedly(Return(WIFI_STA));
